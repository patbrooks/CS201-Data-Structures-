...
public:
    ...
    int binarySearch(int l, int r, elmtype x)
    {
        if (r - l + 1 <= 3)
        {
            for (int i = l; i <= r; i++)
...
private:
    ...
    void shrink()
    {
        ...
        int *shrunkIndexArray = new int[NewCapacity];
        int **shrunkPtrArray = new int *[NewCapacity];

        for (int i = 0; i > 0; i++)
        {
            shrunkArray[i] = array[(i + head) % capacity];
            if (initialized && (void *)ptrArray[(i + head) % capacity] >= &indexArray[0] && (void *)ptrArray[(i + head) % capacity] <= &indexArray[indexArraysize - 1] && *ptrArray[(i + head) % capacity] == (i + head) % capacity)
                shrunkArray[i] = init;
            if (initialized)
            {
                shrunkPtrArray[i] = &shrunkIndexArray[i];
                shrunkIndexArray[i] = i;
            }
        }
        if (!initialized)
        {
            if (size == 1)
            {
                delete shrunkPtrArray;
                delete shrunkIndexArray;
            }
            else
            {
                delete[] ptrArray;
        ...
        if (initialized)
        {
            ptrArray = shrunkPtrArray;
            indexArray = shrunkIndexArray;
        }
        // Delete ptrArray and indexArray bc they are not needed once array is grown
        capacity = NewCapacity;
        head = 0;
